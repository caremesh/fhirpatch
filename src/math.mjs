...
import {FP_TimeBase, FP_Quantity} from './types.mjs';
...
engine.plus = function(xs, ys) {
  if (xs.length == 1 && ys.length == 1) {
    const x = util.valData(xs[0]);
    const y = util.valData(ys[0]);
...
engine.abs = function(x) {
  if (isEmpty(x)) {
    ...
    const num = ensureNumberSingleton(x);
    return Math.abs(num);
  }
};
...
engine.log = function(x, base) {
  if (isEmpty(x) || isEmpty(base)) {
    ...
    const num = ensureNumberSingleton(x);
    const num2 = ensureNumberSingleton(base);
...
engine.power = function(x, degree) {
  if (isEmpty(x) || isEmpty(degree)) {
    ...
    const num = ensureNumberSingleton(x);
    const num2 = ensureNumberSingleton(degree);
...
engine.round = function(x, acc) {
  if (isEmpty(x)) {
    ...
    if (isEmpty(acc)) {
      ...
      const num2 = ensureNumberSingleton(acc);
      const degree = Math.pow(10, num2);
