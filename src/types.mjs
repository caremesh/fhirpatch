...
import dateFns, {addMinutes} from 'date-fns';

const ucumUtils = UcumLhcUtils.getInstance();
export const ucumSystemUrl = 'http://unitsofmeasure.org';
export const timeFormat =
  '[0-9][0-9](\\:[0-9][0-9](\\:[0-9][0-9](\\.[0-9]+)?)?)?(Z|(\\+|-)[0-9][0-9]\\:[0-9][0-9])?';
export const timeRE = new RegExp('^T?'+timeFormat+'$');
export const dateTimeRE = new RegExp(
...
export class FP_Quantity extends FP_Type {
  ...
  equals(otherQuantity) {
    ...
    const normalizedOtherQuantity = FP_Quantity.toUcumQuantity(otherQuantity.value, otherQuantity.unit);
    const convResult = ucumUtils.convertUnitTo(normalizedOtherQuantity.unit, normalizedOtherQuantity.value, thisQuantity.unit);
  ...
  _compareYearsAndMonths(otherQuantity) {
    ...
    const magnitude2 = FP_Quantity._yearMonthConversionFactor[otherQuantity.unit];
...
export const surroundingApostrophesRegex = /^'|'$/g;
/**
...
FP_Quantity.convUnitTo = function(fromUnit, value, toUnit) {
  ...
  const toYearMonthMagnitude = FP_Quantity._yearMonthConversionFactor[toUnit];
  ...
  const toMagnitude = FP_Quantity._calendarDuration2Seconds[toUnit];
...
export class FP_TimeBase extends FP_Type {
  ...
  plus(timeQuantity) {
    ...
    const isIntUnit = FP_Quantity.integerUnits[ucumUnit];
    ...
    if (this._getPrecision() < unitPrecision) {
      const unquotedUnit = ucumUnit.slice(1, ucumUnit.length-1);
      const neededUnit = cls._datePrecisionToUnquotedUcum[
          this._getPrecision()];
      const convResult = ucumUtils.convertUnitTo(unquotedUnit, qVal, neededUnit);
      if (convResult.status != 'succeeded') {
    ...
    const newDate = FP_TimeBase.timeUnitToAddFn[ucumUnit](this._getDateObj(), qVal);
    ...
    if (isTime) {
      precision += 3;
    } // based on dateTimeRE, not timeRE
  ...
  equals(otherDateTime) {
    ...
    if (!(otherDateTime instanceof this.constructor)) {
      ...
      let thisPrec = this._getPrecision();
      let otherPrec = otherDateTime._getPrecision();
      ...
      if (thisPrec == otherPrec) {
        ...
        let commonPrec = thisPrec <= otherPrec ? thisPrec : otherPrec;
        // Adjust for timezone offsets, if any, so they are at a common timezone
        const thisUTCStr = this._getDateObj().toISOString();
        const otherUTCStr = otherDateTime._getDateObj().toISOString();
        ...
        const thisAdj = thisPrec > 2 ? (new FP_DateTime(thisUTCStr))._getTimeParts() : this._getTimeParts();
        const otherAdj = otherPrec > 2 ? (new FP_DateTime(otherUTCStr))._getTimeParts() : otherDateTime._getTimeParts();

        for (let i = 0; i <= commonPrec && rtn !== false; ++i) {
  ...
  equivalentTo(otherDateTime) {
    let rtn = otherDateTime instanceof this.constructor;
    if (rtn) {
      const thisPrec = this._getPrecision();
      const otherPrec = otherDateTime._getPrecision();
   ...
   */
  compare(otherTime) {
    const thisPrecision = this._getPrecision();
    const otherPrecision = otherTime._getPrecision();
    const thisTimeInt = thisPrecision <= otherPrecision ?
      this._getDateObj().getTime(): this._dateAtPrecision(otherPrecision).getTime();
    const otherTimeInt = otherPrecision <= thisPrecision ?
  ...
  _createDate(year, month, day, hour, minutes, seconds, ms, timezoneOffset) {
    let d = new Date(year, month, day, hour, minutes, seconds, ms);
    if (timezoneOffset) {
      ...
      const localTimezoneMinutes = d.getTimezoneOffset();
      let timezoneMinutes = 0; // if Z
      if (timezoneOffset != 'Z') {
        const timezoneParts = timezoneOffset.split(':'); // (+-)hours:minutes
        const hours = parseInt(timezoneParts[0]);
...
FP_TimeBase.timeUnitToAddFn = {
  '\'a\'': dateFns.add_years,
  '\'mo\'': dateFns.add_months,
  '\'wk\'': dateFns.add_weeks,
  '\'d\'': dateFns.add_days,
  '\'h\'': dateFns.add_hours,
  '\'min\'': dateFns.add_minutes,
  '\'s\'': dateFns.add_seconds,
  '\'ms\'': dateFns.add_milliseconds,
};
...
export class FP_DateTime extends FP_TimeBase {
  ...
  _dateAtPrecision(precision) {
    const timeParts = this._getTimeParts();
    const timezoneOffset = this._getMatchData()[7];
    // Get the date object first at the current precision.
    const thisPrecision = this._getPrecision();
    let year = parseInt(timeParts[0]);
    let month = thisPrecision > 0 ? parseInt(timeParts[1].slice(1)) - 1 : 0;
    let day = thisPrecision > 1 ? parseInt(timeParts[2].slice(1)) : 1;
    let hour = thisPrecision > 2 ? parseInt(timeParts[3]) : 0;
    let minutes = thisPrecision > 3 ? parseInt(timeParts[4].slice(1)): 0;
    const seconds = thisPrecision > 4 ? parseInt(timeParts[5].slice(1)): 0;
    const ms = timeParts.length > 6 ? parseInt(timeParts[6].slice(1)): 0;
    let d = this._createDate(year, month, day, hour, minutes, seconds, ms,
        timezoneOffset);
...
export class FP_Time extends FP_TimeBase {
  ...
  _dateAtPrecision(precision) {
    const timeParts = this._getTimeParts();
    const timezoneOffset = this._getMatchData()[4];
    // Get the date object first at the current precision.
    const thisPrecision = this._getPrecision();
    const year = 2010; // Have to pick some year for the date object
    const month = 0;
    const day = 1;
    let hour = parseInt(timeParts[0]);
    let minutes = thisPrecision > 0 ? parseInt(timeParts[1].slice(1)): 0;
    const seconds = thisPrecision > 1 ? parseInt(timeParts[2].slice(1)): 0;
    const ms = timeParts.length > 3 ? parseInt(timeParts[3].slice(1)): 0;
    let d = this._createDate(year, month, day, hour, minutes, seconds, ms,
        timezoneOffset);
    if (timezoneOffset) {
...
FP_DateTime.isoDateTime = function(date, precision) {
  ...
  if (precision > 2) {
    ...
    const tzMin = tzOffset % 60;
    const tzHour = (tzOffset - tzMin) / 60;
 ...
 * @return {boolean|[]}
 */
export function isFn(coll, typeInfo) {
  if (coll.length === 0) {
    return [];
  }
