...
import {FP_Quantity} from './types.mjs';
...
const quantityRegexMap = {value: 1, unit: 5, time: 6};
engine.toQuantity = function(coll, toUnit) {
  ...
  if (coll.length > 1) {
    ...
    if (typeof v === 'number') {
      ...
      const unit = quantityRegexRes[quantityRegexMap.unit];
      const time = quantityRegexRes[quantityRegexMap.time];
...
function defineTimeConverter(timeType) {
  const timeName = timeType.slice(3); // Remove 'FP_'
  engine['to'+timeName] = function(coll) {
    ...
    if (coll.length === 1) {
      const t = types[timeType].checkString(util.valData(coll[0]));
      if (t) {
        rtn = t;
      }
    }
