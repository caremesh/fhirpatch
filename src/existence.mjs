...
function subsetOf(coll1, coll2) {
  ...
  if (rtn) {
    ...
    for (let p=0, pLen=coll1.length; p<pLen && rtn; ++p) {
      ...
      const obj1Str = orderedJsonStringify(obj1);
      let found = false;
      if (p===0) { // c2Hash is not yet built
        for (let i=0, len=coll2.length; i<len; ++i) {
          ...
          const obj2 = util.valData(coll2[i]);
          const obj2Str = orderedJsonStringify(obj2);
          c2Hash[obj2Str] = obj2;
...
engine.distinctFn = function(x) {
  ...
  if (x.length > 0) {
    ...
    for (let i=0, len=x.length; i<len; ++i) {
      const xObj = x[i];
      const xStr = JSON.stringify(xObj);
      const uObj = uniqueHash[xStr];
      if (uObj === undefined) {
        unique.push(xObj);
